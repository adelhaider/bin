...
class CodeQualityReportGenerator {
    ...
    def processFileDetails(path, name) {
        if (path.startsWith("${baseFolder}/Yodel_OSB")) {
            def relativePath = path.replace("${baseFolder}/Yodel_OSB", "").substring(1)
        ...
        } else {//path starts with $baseFolder/Metadata
            //e.g. OSB/Metadata/Maven/Common
            def relativePath = path.replace("${baseFolder}", "").substring(1)
            println "path = ${path} relativePath = ${relativePath}"
            def relativePathParts = relativePath.split('/')
            return [
                      artifactId: name.split('\\.')[0]
                    , path: relativePath + '/' + name
                    , group: relativePathParts[2]
                ]
        }
    }
